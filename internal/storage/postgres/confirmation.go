...
func (db *DB) CreateConfirmation(accountID int64, t domain.ConfirmationType) (*domain.Confirmation, error) {
	tx, err := db.Beginx()
	if err != nil {
		return nil, err
	}
	var ecc domain.Confirmation
...
func (db *DB) PendingConfirmationByKey(key string) (*domain.Confirmation, error) {
...
func (db *DB) FailedConfirmationIncrease(id int64) (*domain.Confirmation, error) {
	var c domain.Confirmation
...
func (db *DB) Confirm(id int64) (*domain.Confirmation, error) {
	tx, err := db.Beginx()
	if err != nil {
		return nil, err
	}
