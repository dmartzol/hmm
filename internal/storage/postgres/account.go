...
func (db *DB) Account(id int64) (*domain.Account, error) {
...
func (db *DB) Accounts() (domain.Accounts, error) {
...
func (db *DB) AccountWithCredentials(email, allegedPassword string) (*domain.Account, error) {
	var a domain.Account
...
func (db *DB) CreateAccount(first, last, email, password, confirmationCode string, dob time.Time, gender, phone *string) (*domain.Account, *domain.Confirmation, error) {
	tx, err := db.Beginx()
	if err != nil {
		return nil, nil, err
	}
	...
	var cc domain.Confirmation
	sqlStatement = `insert into confirmations (type, account_id, key, confirmation_target) values ($1, $2, $3, $4) returning *`
	err = tx.Get(&cc, sqlStatement, domain.ConfirmationTypeEmail, a.ID, confirmationCode, email)
	if err != nil {
		tx.Rollback()
		return nil, nil, err
	}
